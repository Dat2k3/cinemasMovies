<?php
    ...
    if (isset($_POST['name']))
    {
        ...
        if (empty($name)) {
            $error = 'Please enter the name of event';
        }
        else if ($_FILES['image']['error'] != UPLOAD_ERR_OK) {
            $error = 'Please upload the image of event you want display';
        }
        else {
            if(!check_exist_name($name, 'event')) {
                upload_event($name, $_FILES['image']['name']);
                $result = upload_event_image($_FILES['image']);
                if($result['code'] == 0) {
                ...
                $error = 'The event title is existed, please enter the name of event again';
            }
        }
    }
?>
...
<head>
    ...
    <div class="tilte-login" style="background-color: #EFEBDB;">
        <h5 class="text-center my-auto pt-2 pb-2"><strong>ADD THE EVENT OF PROMOTION </strong></h5>
    </div>
</head>
            ...
            <div class="col-lg-4 col-md-5 form-poster mt-4 mb-4 pl-5 pr-5 pt-4 pb-2">
                ...
                <form method="post" novalidate enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <input value="<?= $name ?>" name="name" required class="form-control" type="text" placeholder="Name of promotion event" id="name">
                    </div>
