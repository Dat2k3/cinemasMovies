<?php
    ...
    if (isset($_POST['nameCurrent']) && isset($_POST['name']) && isset($_POST['time']) && isset($_POST['detail']))
    {
        ...
        if (empty($nameCurrent)) {
            ...
            if(check_exist_name($nameCurrent, 'now_films')) {
                $result = update_film($name, $time, $detail, $nameCurrent, 'now_films');
                if($result['code'] == 0) {
...
<head>
    ...
    <title>Modify movies now showing</title>
    ...
    <div class="tilte-login" style="background-color: #EFEBDB;">
        <h5 class="text-center my-auto pt-2 pb-2"><strong>MODIFY THE MOVIE NOW SHOWING</strong></h5>
    </div>
</head>
