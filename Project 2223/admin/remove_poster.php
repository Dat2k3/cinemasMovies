<?php
    ...
    if (isset($_POST['name']))
    {
        ...
        if (empty($name)) {
            $error = 'Please enter name of poster';
        }
        else {
            if(check_exist_name($name, 'poster')) {
                $name_film = name_image($name, 'poster')['image'];
                $result = remove_film($name, 'poster');
                remove_image_film($name_film, 'poster');
                if($result['code'] == 0) {
...
<head>
    ...
    <div class="tilte-login" style="background-color: #EFEBDB;">
        <h5 class="text-center my-auto pt-2 pb-2"><strong>REMOVE THE POSTER OF MOVIE</strong></h5>
    </div>
</head>
            ...
            <div class="col-lg-4 col-md-5 form-poster mt-4 mb-4 pl-5 pr-5 pt-4 pb-2">
                ...
                <form method="post" novalidate enctype="multipart/form-data">
                    <div class="form-group">
                        ...
                        <input value="<?= $name ?>" name="name" required class="form-control" type="text" placeholder="Name of poster you want to remove" id="name">
                    </div>
