<?php
    ...
    if (isset($_POST['nameCurrent']) && isset($_POST['name']))
    {
        $name = $_POST['name'];
        $nameCurrent = $_POST['nameCurrent'];
   
        if (empty($nameCurrent)) {
            $error = 'Please enter current name of event';
        }
        else if (empty($name)) {
            $error = 'Please enter name of event you want to change';
        }
        else {
            if(check_exist_name($nameCurrent, 'event')) {
                $result = update_event($name, $nameCurrent);
                if($result['code'] == 0) {
                ...
                $error = 'The event title is not found';
            }
        }
    }
?>
...
<head>
    ...
    <div class="tilte-login" style="background-color: #EFEBDB;">
        <h5 class="text-center my-auto pt-2 pb-2"><strong>MODIFY THE EVENT OF PROMOTION</strong></h5>
    </div>
</head>
            ...
            <div class="col-lg-4 col-md-5 form-poster mt-4 mb-4 pl-5 pr-5 pt-4 pb-2">
                ...
                <form method="post">
                    <div class="form-group">
                        <label for="nameCurrent"><strong>Event current name</strong></label>
                        <input value="<?= $nameCurrent ?>" name="nameCurrent" required class="form-control" type="text" placeholder="Current name of event" id="name">
                    </div>

                    <div class="form-group">
                        <label for="name"><strong>New event name</strong></label>
                        <input value="<?= $name ?>" name="name" required class="form-control" type="text" placeholder="New name of event" id="name">
                    </div>
