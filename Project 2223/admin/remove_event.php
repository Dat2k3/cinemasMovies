    ...
    if (isset($_POST['name']))
    {
        ...
        if (empty($name)) {
            ...
            if(check_exist_name($name, 'event')) {
                $name_film = name_image($name, 'event')['image'];
                $result = remove_film($name, 'event');
                remove_image_film($name_film, 'event');
                if($result['code'] == 0) {
...
<head>
    ...
    <div class="tilte-login" style="background-color: #EFEBDB;">
        <h5 class="text-center my-auto pt-2 pb-2"><strong>REMOVE THE EVENT OF PROMOTION</strong></h5>
    </div>
</head>
