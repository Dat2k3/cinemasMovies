<?php
    ...
    if (isset($_POST['nameCurrent']) && isset($_POST['name']) && isset($_POST['position']))
    {
        $name = $_POST['name'];
        ...
        if (empty($nameCurrent)) {
            $error = 'Please enter current name of poster';
        }
        else if (empty($name)) {
            $error = 'Please enter name of poster you want to change';
        }
        else if (empty($position)) {
            $error = 'Please enter the position of poster';
        }
        else {
            if(check_exist_name($nameCurrent, 'poster')) {
                $result = update_poster($name, $position, $nameCurrent);
                if($result['code'] == 0) {
                ...
                $error = 'The poster title is not found';
            }
        }
    }
?>
...
<head>
    ...
    <div class="tilte-login" style="background-color: #EFEBDB;">
        <h5 class="text-center my-auto pt-2 pb-2"><strong>MODIFY THE POSTER OF MOVIE</strong></h5>
    </div>
</head>
            ...
            <div class="col-lg-4 col-md-5 form-poster mt-4 mb-4 pl-5 pr-5 pt-4 pb-2">
                ...
                <form method="post">
                    <div class="form-group">
                        <label for="nameCurrent"><strong>Poster current name</strong></label>
                        <input value="<?= $nameCurrent ?>" name="nameCurrent" required class="form-control" type="text" placeholder="Current name of poster" id="name">
                    </div>

                    <div class="form-group">
                        <label for="name"><strong>New poster name</strong></label>
                        <input value="<?= $name ?>" name="name" required class="form-control" type="text" placeholder="New name of poster" id="name">
                    </div>

                    <div class="form-group">
                        <label for="time"><strong>New position</strong></label>
