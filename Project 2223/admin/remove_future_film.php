    ...
    if (isset($_POST['name']))
    {
        ...
        if (empty($name)) {
            ...
            if(check_exist_name($name, 'future_films')) {
                $name_film = name_image($name, 'future_films')['image'];
                $result = remove_film($name, 'future_films');
                remove_image_film($name_film, 'future_films');
                if($result['code'] == 0) {
...
<head>
    ...
    <title>Remove movies coming soon</title>
    ...
    <div class="tilte-login" style="background-color: #EFEBDB;">
        <h5 class="text-center my-auto pt-2 pb-2"><strong>REMOVE THE MOVIE COMING SOON</strong></h5>
    </div>
</head>
